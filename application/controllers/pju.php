...
class Pju extends CI_Controller {
    ...
    public function index(){
        ...
        $this->load->view('pju/polling');
    }

    public function registrasi(){
        
        $data['title'] = "Registrasi";
        $this->load->template('pju/registrasi',$data);
    }
    ...
    function verifykode() {
        ...
        if($this->form_validation->run() == FALSE) {
            ...
            $this->load->template('pju/Verifikasi');
        }
        else {

            redirect('polling/pju', 'refresh');
        }
    }
    ...
    function send(){
        ...
        $this->load->template('pju/verifikasi',$data);
    }
    ...
    function verifikasi(){
        ...
        if($this->form_validation->run()==true){
            ...
            $cekvote=$this->m_register->cekvotepju($mobile); 
            
        if($cekvote->num_rows()>0){ 

            $no = $cekvote->row_array();

            redirect('pju/edit/'.$no['polling_id']);
    ...
    function edit($id){
        ...
        if($this->form_validation->run() == true){
            ...
            redirect('polling/hasil_pju', $data);
        }else{

            $data['title'] = "Polling Pju";
            $data['soal_pju'] = $this->m_polling->getKuisionerPju();
            $data['jawaban'] = $this->m_polling->getJawaban();
            $data['pju']=$this->m_polling->cekpolling($id)->row_array();
            $data['message']="";
            $this->load->template('pju/edit',$data);
        }
