...
class Artikel extends CI_Controller {
	...
	function index(){
        ...
        if($this->uri->segment(4)=="delete_success")
            ...
            $data['message']='';
            $this->load->template_superadmin('superadmin/artikel/index',$data);
    }
    ...
    function tambah(){
        ...
        if($this->form_validation->run()==true){
            ...
            $data['message']="";
            $this->load->template_superadmin('superadmin/artikel/tambah',$data);
        }
    }
    ...
    function edit($id){
        ...
        if($this->form_validation->run()==true){
            ...
            $this->m_artikel->update($id,$info);
            
            //tampilkan pesan
            $data['message']="<div class='alert alert-success'>Data Berhasil diupdate</div>";
            
            //tampilkan data anggota 
            $data['artikel']=$this->m_artikel->cekId($id)->row_array();
           	$this->load->template_superadmin('superadmin/artikel/edit',$data);
        }else{
            $data['artikel']=$this->m_artikel->cekId($id)->row_array();
            $data['message']="";
            $this->load->template_superadmin('superadmin/artikel/edit',$data);
    ...
    function hapus(){
        $article_id=$this->input->post('article_id');
        $detail=$this->m_artikel->cek($article_id)->result();
        $this->m_artikel->hapus($article_id);
    }
